...
const validationSchema = Joi.object({
    ...
    coCcaptainEmail : Joi.string().email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }).trim().min(16).max(50).required(),
    ...
    additionalMembers : Joi.object({
        ...
        email : Joi.string().email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }).trim().min(16).max(50).required(),
    }),
    payment : Joi.object({
