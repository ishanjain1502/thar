...
export default function MUNSlider() {
  ...
  let committees = [
    ...
    },
    {
      ...
      agenda:
        "Preservation of magic community from the mugggle borns with special emphasis on University students.",
      chair: [
        {
          name: "GARIMA RAJPAL",
        },
        {
          name: "ABHIYANKER SHUKLA",
        },
      ],
    },
    {
      ...
      <div className="w-10/12 md:w-8/12 xl:w-8/12 flex flex-col-reverse lg:flex-row">
        ...
        >
          {arr.map((i, index) => (
            <div
              key={index}
              className={`flex justify-center items-center flex-col md:flex-row overflow-hidden text-black ${
                index % 2 === 0 ? "md:flex-row-reverse" : "md:flex-row"
              }`}
            >
              <Image
                src={`/assets/images/mun/comittees/${committees[index].name}.png`}
                alt=""
                quality={100}
                width={280}
                height={280}
              ...
              <div>
                ...
                <p className="text-base 2xl:text-lg leading-tight tracking-tight">
                  {committees[index].desc}
                </p>
                {(committees[index].agenda || committees[index].chair) && (
